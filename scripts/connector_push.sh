#!/usr/bin/env bash
# Connector-push pre-flight (D1/C2). Asks IgniteIQ to copy the pinned connector
# image into this project's depot-connectors repository, to which this project's
...
# image — bytes flow IgniteIQ → customer, authorized by a grant the customer
# made on their own repo.
set -euo pipefail

echo "[connector-push] requesting Depot connector image → ${REGION}-docker.pkg.dev/${PROJECT_ID}/${TARGET_REPO}/${IMAGE_NAME}:${IMAGE_TAG}"

body=$(cat <<JSON
{"orgId":"${ORG_ID}","projectId":"${PROJECT_ID}","region":"${REGION}","targetRepo":"${TARGET_REPO}","imageName":"${IMAGE_NAME}","imageTag":"${IMAGE_TAG}"}
JSON
)
...
for attempt in 1 2 3 4 5; do
  code=$(curl -s -o /tmp/connector_push_resp.json -w "%{http_code}" --max-time 120 \
    ...
    -d "${body}" || echo "000")
  if [[ "${code}" == "200" ]]; then
    echo "[connector-push] image published into ${TARGET_REPO}."
    exit 0
  fi
